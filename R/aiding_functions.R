    ...
    for(parent2 in names(P2_gamete$gamete)){
      ...
      if(nrow(result) > 0){
        ...
        result$P1_freq <- sapply(1:nrow(result),function(r){
          table(P1_G)[paste0(sort(result[r,1:(ploidy/2)]),collapse = '')]/length(P1_G)
        })
        result$P2_freq <- sapply(1:nrow(result),function(r){
          table(P2_G)[paste0(sort(result[r,(ploidy/2 + 1):(ploidy)]),collapse = '')]/length(P2_G)
        })
...
#'
#' @examples
sharing_identification <- function(mrk){
